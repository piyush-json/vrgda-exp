...
export async function fetchTokenData(tokenAddress: string) {
  try {
    const response = await fetch(
      `${BIRDEYE_API_BASE_URL}/defi/token_overview?address=${tokenAddress}`,
      {
        method: "GET",
 ...
 * @param tokenAddress The Solana token address
 * @param timeframe Timeframe for price history (e.g., '1H', '1D', '1W', '1M')
 */
...
export async function fetchTokenPriceHistory(
  tokenAddress: string,
  timeframe: "30m" | "1h" | "2h" | "4h" | "8h" | "24h" = "24h"
) {
  try {
    const response = await fetch(
      `${BIRDEYE_API_BASE_URL}/defi/v3/token/trade-data/single?address=${tokenAddress}`,
      {
        method: "GET",
        headers: {
          ...defaultHeaders,
...
/**
 * Fetch top tokens by market cap from Birdeye API
 * @param limit Number of tokens to fetch
 * @param offset Pagination offset
 */
interface TokenListParams {
  sort_by?: 'liquidity' | 'market_cap' | 'volume_24h_usd' | 'holder';
  sort_type?: 'asc' | 'desc';
...
export async function fetchTopTokens({
  ...
  try {
    ...
    if (min_liquidity) params.append('min_liquidity', min_liquidity.toString());
    if (max_liquidity) params.append('max_liquidity', max_liquidity.toString());
    if (min_market_cap) params.append('min_market_cap', min_market_cap.toString());
    if (max_market_cap) params.append('max_market_cap', max_market_cap.toString());
    if (min_volume_24h_usd) params.append('min_volume_24h_usd', min_volume_24h_usd.toString());
    if (min_holder) params.append('min_holder', min_holder.toString());

    const response = await fetch(
      `${BIRDEYE_API_BASE_URL}/defi/v3/token/list?${params}`,
      {
        method: "GET",
        headers: {
          ...defaultHeaders,
