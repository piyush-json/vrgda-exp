...
impl PreciseNumber {
  ...
  pub fn precise_supply_amt(amt: u64, mint_decimals: u8) -> PreciseNumber {
    PreciseNumber {
      value: InnerUint::from(amt)
        // ────────────────────────────────────────────────────────┐
        // 1) “Normalize” amt up to a 12-decimal supply scale:
        //    if mint.decimals = D, then multiply by 10^(12−D).
        .checked_mul(
          InnerUint::from(
            get_u128_pow_10(12_u8 - mint_decimals)
          )
        )
        .unwrap()
        // ────────────────────────────────────────────────────────┘
        // 2) Then tack on 6 more decimal places (×10^6)
        //    so that overall you’ve multiplied by 10^(18−D):
        //      amt × 10^(12−D) × 10^6  ==  amt × 10^(18−D)
  ...
  pub fn clamp(self, min: PreciseNumber, max: PreciseNumber) -> PreciseNumber {
    // 1) If zero or negative, enforce your floor:
    if self.less_than_or_equal(&PreciseNumber::zero()) {
        return min;
    }
    // 2) If above your ceiling, cap it:
    if self.greater_than(&max) {
        return max;
    }
    // 3) Otherwise (0 < self <= max) return the real price:
    self
}
...
pub fn to_mint_amount(
  amt: &PreciseNumber,
  mint_decimals: u8,
  ceil: bool,
) -> u64 {
  let pow_10 = get_pow_10(mint_decimals);       // e.g. 10⁹
  // 1) multiply wad‐scaled amt by 10^decimals
  let scaled = amt.checked_mul(&pow_10).unwrap();
  // 2) divide by ONE_PREC (10^18) to get raw token units
  let unwad = scaled.checked_div(&ONE_PREC).unwrap();
  // 3) round
  let rounded = if ceil {
      unwad.ceiling().unwrap()
  } else {
      unwad.floor().unwrap()
  };
  // 4) drop back to u64
  rounded.checked_div(&ONE_PREC).unwrap().to_imprecise().unwrap() as u64
}

pub fn to_actual_mint_amount(
  amt: &PreciseNumber,
) -> u64 {
   amt.to_imprecise().unwrap() as u64
}
...
pub fn scale_price_for_transfer(price: PreciseNumber, decimals: u8) -> u64 {
  // 1) Compute 10^decimals
  let pow10 = InnerUint::from(10u128.pow(decimals as u32));

  msg!("pow10: {}", pow10);
  // 2) Multiply wad‐price by 10^decimals
  let scaled_up = price
      .value
      .checked_mul(pow10)
      .unwrap();

  msg!("scaled_up: {}", scaled_up);
  // 3) Divide by 10^18 to shift out the wad
  let atomic = PreciseNumber{
    value: scaled_up
    .checked_div(ONE_PREC.value)  // ONE_PREC.value == 10^18
    .unwrap()
  };
  
  msg!("atomic: {}", atomic.value);
  // 4) Convert into u64
  atomic.value.as_u64()
}
