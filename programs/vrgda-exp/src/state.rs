...
use math::utils::{VrgdaSchedule, Wad, WAD};

use crate::{error::VrgdaResult, math::{precise_number::PreciseNumber, precise_supply_amt, InnerUint, SignedPreciseNumber, HALF, ONE, ONE_PREC, U192}};
use super::*;
...
impl VRGDA {
    ...
    pub fn vrgda_price(&self, now: i64, sold: u64) -> VrgdaResult<PreciseNumber> {
        ...
        let time_since_start = PreciseNumber::new((now - self.vrgda_start_timestamp) as u128).unwrap();
            // .checked_mul(&ONE_PREC)
            // .unwrap();
    ...
    pub fn vrgda_price_for_amount(
        ...
        let rt = ((now -  self.vrgda_start_timestamp) as u64)
                        .saturating_mul(r);
        ...
        msg!("RT MINUS N PRECISE (ideal tokens to be sold): {:?}", precise_rt_minus_n);
        ...
        let raw_exp = ln1k
            .checked_mul(&t_minus_sr).ok_or(VRGDAError::ExponentError)?;

        msg!("RAW EXPONENT: {:?}", raw_exp);

        let next_mul = raw_exp
            .exp().ok_or(VRGDAError::ExponentTooLarge)?;
        ...
        let q = neg_ln1k
            .checked_mul(&inv_r).ok_or(VRGDAError::ExponentError)?
            .exp().ok_or(VRGDAError::ExponentTooLarge)?;
        msg!("q: {:?}", q);
    ...
    pub fn vrgda_price_for_amount_v1(
        ...
        amount: u64,
         ...
         msg!("q: {:?}", q);
        // ───────────────────────────────────────────────────────────────────────────
        // 9) Chunked geometric‐series sum so q^m never overflows:
        ...
        while remaining > 0 {
            ...
            let batch_cost  = current_price
                .checked_mul(&numerator).unwrap()
                .checked_div(&denominator).ok_or(VRGDAError::DivisionError)?;
